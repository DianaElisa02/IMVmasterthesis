...
1. Continuous event-study specifications:
   Y_hrt = sum_{tau != 2019} beta_tau (1[t=tau] x Exposure_r)
           + region FE + year FE + controls + error

2. Continuous placebo DiD using pre-reform years only:
   Y_hrt = beta_placebo (1[t=2019] x Exposure_r)
           + region FE + year FE + controls + error

3. Tercile event-study specifications:
   Y_hrt = sum_{tau != 2019} beta_tau^M (1[t=tau] x Medium_r)
         + sum_{tau != 2019} beta_tau^H (1[t=tau] x High_r)
         + region FE + year FE + controls + error

4. Tercile placebo DiD using pre-reform years only:
   Y_hrt = beta_M (1[t=2019] x Medium_r)
         + beta_H (1[t=2019] x High_r)
         + region FE + year FE + controls + error

5. Region-specific linear time trend event-study robustness.

All primary estimation uses unweighted PyFixest OLS, region and year fixed effects,
CRV1 standard errors clustered by Autonomous Community, and wild cluster bootstrap
p-values for individual coefficients.

Important interpretation:
Event-study and placebo tests provide evidence consistent or inconsistent with
parallel pre-trends. They do not prove the parallel trends assumption.
"""
...
def _extract_boot_pvalue(boot) -> float:
    """
    Extract p-value from PyFixest wildboottest output across possible versions.
    """
...
def _wald_joint_test(fit, terms: list[str]) -> tuple[float, float]:
    """
    Cluster-robust joint Wald test for selected terms.
    This is not a wild-bootstrap joint test.
    """
    ...
    try:
        ...
        logger.warning("Joint Wald test failed for terms %s: %s", present_terms, exc)
        return np.nan, np.nan
...
def make_region_terciles(
    ...
    Exposure is expected to be time-invariant at the regional level. To be robust
    to repeated household observations, the function first collapses to one
    exposure value per region using the regional mean.

    Returns one row per region with:
        drgn2, exposure_value, exposure_tercile, medium_exp, high_exp
    """
    ...
    # qcut can fail with duplicate values. Rank first to ensure stable groups.
...
def build_event_study_data(
    ...
    Reference year is 2019. The interaction terms are created for all years
    in EVENT_STUDY_YEARS, which should exclude 2019 and 2020.
    """
    if exposure not in panel.columns:
        raise ValueError(f"Exposure variable '{exposure}' not found in panel.")
...
def run_event_study(
    ...
    result["lead_mean"] = float(np.mean(lead_coefs)) if lead_coefs else np.nan
    result["lead_max_abs"] = float(np.max(np.abs(lead_coefs))) if lead_coefs else np.nan
...
def run_placebo_continuous(
    ...
    Fake post is 2019. Exposure is explicit, not hard-coded.
    """
    if exposure not in panel.columns:
        raise ValueError(f"Exposure variable '{exposure}' not found in panel.")
    ...
    return pd.DataFrame([{
        ...
        "verdict": (
            "consistent_with_parallel_pretrends"
            if not np.isnan(p_wbt) and p_wbt > 0.10
            else "pretrend_concern"
...
def build_tercile_event_study_data(
    ...
    """
    Build event-study data using exposure terciles.

    Low exposure is omitted. Medium and high are compared against low.
    Terciles are computed automatically from region-level exposure values.
    """
    keep_years = sorted(set(PRE_YEARS + EVENT_YEARS + [REF_YEAR]))
...
def run_event_study_terciles(
    ...
    """
    Estimate tercile event-study model.

    Low exposure tercile is the omitted group.
    """
    controls = _available_controls(df, controls)
    ...
    # Add reference rows for plotting/table clarity.
    for group in ["medium", "high"]:
        rows.append({
            "model": "tercile_event_study",
            "exposure_spec": exposure,
...
def run_placebo_terciles(
    ...
    """
    Estimate placebo DiD using exposure terciles.

    Low exposure tercile is omitted.
    """
    ...
    # Joint placebo Wald: medium = high = 0.
